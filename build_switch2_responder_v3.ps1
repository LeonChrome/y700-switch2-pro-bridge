...
$Src = Join-Path $Root "src\Switch2FfsResponderV3.java"
$Build = Join-Path $Root "build\switch2_responder_v3"
...
& $Javac -source 8 -target 8 -Xlint:-options -d $Classes $Src
if ($LASTEXITCODE -ne 0) { throw "javac failed: $LASTEXITCODE" }

$env:JAVA_HOME = $JavaHome
$env:Path = (Join-Path $JavaHome "bin") + ";" + $env:Path
$ClassFiles = Get-ChildItem -LiteralPath $Classes -Recurse -Filter "Switch2FfsResponderV3*.class" |
