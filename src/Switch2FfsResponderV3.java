...
public class Switch2FfsResponderV3 {
    ...
    private static final File LOG_FILE = new File("/data/local/tmp/switch2_ffs_responder_v3.log");
    private static final File HID_OUTPUT_LOG_FILE = new File("/data/local/tmp/switch2_hid_output_v3.log");
    private static final File HID_LAST_OUTPUT_FILE = new File("/data/local/tmp/switch2_last_hid_output_v3.txt");
    ...
    private static final File HAPTIC_LOG_ONLY_FLAG_FILE = new File("/data/local/tmp/switch2_haptic_log_only_v3");
    private static final File HAPTIC_PRESET_FALLBACK_FLAG_FILE = new File("/data/local/tmp/switch2_haptic_preset_fallback_v3");
    ...
    private static void run(String[] args) throws Exception {
        ...
        log("Switch2FfsResponderV3 starting, ffsDir=" + ffsDir + ", hidPath=" + hidPath);
        ...
        touch("/data/local/tmp/switch2_ffs_v3_ready");
    ...
    private static void bridgeBulkOutputToBle(byte[] cmd) {
        ...
        if (isSwitch2HidRumbleReport(cmd)) {
            ...
            if (HAPTIC_PRESET_FALLBACK_FLAG_FILE.exists()) {
                writeBlePreset(active ? PRESET_SHORT_LOW : PRESET_STOP,
                        active ? "bulk-hid-report-active" : "bulk-hid-report-stop");
            } else if (active) {
                writeBleHdStream(cmd, "bulk-hid-report-active");
            } else {
                writeBleHdStop("bulk-hid-report-stop");
            }
            return;
        }
    ...
    private static void writeBlePreset(int preset, String reason) {
        ...
        String command = String.format(Locale.US,
                "cmd 0a91010200040000%02x000000%n", value);
    ...
    private static void writeBleHdStream(byte[] report, String reason) {
        byte[] left = encodeBleVibrationFromSwitchFrame(report, 2);
        byte[] right = encodeBleVibrationFromSwitchFrame(report, 0x12);
        String command = "hdstream " + hexCompact(left) + hexCompact(right) + "\n";
        if (HAPTIC_LOG_ONLY_FLAG_FILE.exists()) {
            log("HD rumble bridge " + reason + " suppressed by log-only mode " + command.trim());
            return;
        }
        try {
            writeText(BLE_WRITE_FILE, command);
            log("HD rumble bridge " + reason + " wrote " + command.trim() +
                    " decoded=" + rumbleDecodedKey(report));
        } catch (Throwable t) {
            log("HD rumble bridge failed " + reason + ": " + t);
        }
    }

    private static void writeBleHdStop(String reason) {
        try {
            writeText(BLE_WRITE_FILE, "hdstop\n");
            log("HD rumble bridge " + reason + " wrote hdstop");
        } catch (Throwable t) {
            log("HD rumble bridge stop failed " + reason + ": " + t);
        }
    }

    private static byte[] encodeBleVibrationFromSwitchFrame(byte[] report, int offset) {
        if (report.length < offset + 5) {
            return buildBleVibrationData(0x0e1, false, 0, 0x1e1, false, 0);
        }
        int b0 = u(report[offset]);
        int b1 = u(report[offset + 1]);
        int b2 = u(report[offset + 2]);
        ...
        return buildBleVibrationData(lowFreq, false, mapSwitchAmpToBle(lowAmp),
                highFreq, false, mapSwitchAmpToBle(highAmp));
    }
    ...
    private static int mapSwitchAmpToBle(int value) {
        return clamp((value * 1023 + 14500) / 29000, 0, 1023);
    }
    ...
    private static final class HidOutputLoop implements Runnable {
        ...
        private long lastHdWriteMs;
        private String lastHdFrameKey = "";
        ...
        private void bridgeHidOutputToBlePreset(byte[] report) {
            ...
            if (!HAPTIC_PRESET_FALLBACK_FLAG_FILE.exists()) {
                if (active) {
                    ...
                    String frameKey = rumbleFrameKey(report);
                    if (now - lastHdWriteMs >= 12 || !frameKey.equals(lastHdFrameKey)) {
                        lastHdWriteMs = now;
                        lastHdFrameKey = frameKey;
                        if (!bridgedRumbleActive) {
                            bridgedRumbleActive = true;
                            rumbleStartMs = now;
                            log("HD HID rumble event start frame=" + frameKey +
                                    " decoded=" + rumbleDecodedKey(report));
                        }
                        writeBleHdStream(report, "hid-out-hd");
                    }
                } else if (bridgedRumbleActive) {
                    bridgedRumbleActive = false;
                    int duration = (int) (now - rumbleStartMs);
                    log("HD HID rumble event stop durationMs=" + duration +
                            " activeReportAgeMs=" + (lastActiveReportMs == 0 ? -1 : now - lastActiveReportMs));
                    writeBleHdStop("hid-out-hd-stop");
                    lastHdFrameKey = "";
                }
                return;
            }
        ...
        private void writeBlePreset(int preset, String reason) {
            Switch2FfsResponderV3.writeBlePreset(preset, reason);
        }
    }
