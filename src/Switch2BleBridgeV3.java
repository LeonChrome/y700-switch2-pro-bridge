...
public class Switch2BleBridgeV3 {
    ...
    private static final File LOG_FILE = new File("/data/local/tmp/switch2_ble_bridge_v3.log");
    private static final File RAW_FILE = new File("/data/local/tmp/switch2_ble_input_raw_v3.log");
    private static final File BUTTON_FILE = new File("/data/local/tmp/switch2_button_changes_v3.log");
    private static final File BLE_WRITE_FILE = new File("/data/local/tmp/switch2_ble_write_v3.txt");

    private static final UUID SERVICE_MAIN = UUID.fromString("ab7de9be-89fe-49ad-828f-118f09df7fd0");
    private static final UUID CHAR_INPUT = UUID.fromString("ab7de9be-89fe-49ad-828f-118f09df7fd2");
    private static final UUID CHAR_TELEMETRY = UUID.fromString("7492866c-ec3e-4619-8258-32755ffcc0f8");
    ...
    private static volatile boolean hdStreamActive;
    private static volatile long hdStreamUntilMs;
    private static volatile byte[] hdLeftVibration = buildBleVibrationData(0x0e1, false, 0, 0x1e1, false, 0);
    private static volatile byte[] hdRightVibration = buildBleVibrationData(0x0e1, false, 0, 0x1e1, false, 0);
    private static final Object hdLock = new Object();
    private static int hdPacketId;
    ...
    private static void run(String[] args) throws Exception {
        ...
        log("Switch2BleBridgeV3 starting");
        ...
        Thread hdThread = new Thread(new HdRumbleLoop(), "hd-rumble-stream");
    ...
    private static void sendBleWriteLine(String line) {
        ...
        if ("play-hd".equalsIgnoreCase(trimmed) || "hd-test".equalsIgnoreCase(trimmed)) {
            startHapticTest("hd");
            return;
        }
        if ("hdstop".equalsIgnoreCase(trimmed) || "stop-hd".equalsIgnoreCase(trimmed)) {
            stopHdStream("file-command");
            return;
        }
        ...
        String target = line.substring(0, space).trim();
        if ("hdstream".equalsIgnoreCase(target)) {
            updateHdStream(hexBytes(line.substring(space + 1)), "file-command");
            return;
        }
        if ("hdonce".equalsIgnoreCase(target)) {
            byte[] data = hexBytes(line.substring(space + 1));
            if (data.length == 10) {
                data = buildPro2HdPacket(nextHdPacketId(), slice(data, 0, 5), slice(data, 5, 5));
            }
            sendBleWrite(CHAR_WRITE_CC48, data);
            return;
        }
        UUID uuid = parseWriteUuid(target);
        if (uuid == null) {
    ...
    private static void sendBleWriteQuiet(UUID uuid, byte[] data, String label) {
        BluetoothGatt gatt = currentGatt;
        if (gatt == null) {
            log(label + " skipped, no current GATT uuid=" + uuid);
            return;
        }
        BluetoothGattService service = gatt.getService(SERVICE_MAIN);
        if (service == null) {
            log(label + " skipped, main service missing uuid=" + uuid);
            return;
        }
        BluetoothGattCharacteristic ch = service.getCharacteristic(uuid);
        if (ch == null) {
            log(label + " skipped, char missing uuid=" + uuid);
            return;
        }
        writeCharacteristicNoResponseQuiet(gatt, ch, data, label);
    }

    private static void updateHdStream(byte[] data, String reason) {
        if (data.length != 10) {
            log("HD stream ignored, expected 10 bytes left5+right5 reason=" + reason +
                    " n=" + data.length + " data=" + hex(data, data.length));
            return;
        }
        synchronized (hdLock) {
            hdLeftVibration = slice(data, 0, 5);
            hdRightVibration = slice(data, 5, 5);
            hdStreamUntilMs = System.currentTimeMillis() + 180;
            hdStreamActive = true;
        }
        log("HD stream update reason=" + reason + " left=" + hex(hdLeftVibration, 5) +
                " right=" + hex(hdRightVibration, 5));
    }

    private static void stopHdStream(String reason) {
        synchronized (hdLock) {
            hdLeftVibration = buildBleVibrationData(0x0e1, false, 0, 0x1e1, false, 0);
            hdRightVibration = buildBleVibrationData(0x0e1, false, 0, 0x1e1, false, 0);
            hdStreamUntilMs = 0;
            hdStreamActive = false;
        }
        for (int i = 0; i < 3; i++) {
            sendBleWrite(CHAR_WRITE_CC48, buildPro2HdPacket(nextHdPacketId(), hdLeftVibration, hdRightVibration));
            sleep(20);
        }
        log("HD stream stop reason=" + reason);
    }

    private static int nextHdPacketId() {
        synchronized (hdLock) {
            int out = hdPacketId & 0x0f;
            hdPacketId++;
            return out;
        }
    }

    private static byte[] buildPro2HdPacket(int packetId, byte[] leftVibration, byte[] rightVibration) {
        byte[] zero = buildBleVibrationData(0x0e1, false, 0, 0x1e1, false, 0);
        byte[] out = new byte[33];
        out[0] = 0x00;
        writeMotorBlock(out, 1, packetId, leftVibration, zero, zero);
        writeMotorBlock(out, 17, packetId, rightVibration, zero, zero);
        return out;
    }

    private static void writeMotorBlock(byte[] out, int offset, int packetId,
            byte[] first, byte[] second, byte[] third) {
        out[offset] = (byte) (0x50 + (packetId & 0x0f));
        System.arraycopy(first, 0, out, offset + 1, 5);
        System.arraycopy(second, 0, out, offset + 6, 5);
        System.arraycopy(third, 0, out, offset + 11, 5);
    }

    private static byte[] buildBleVibrationData(int lfFreq, boolean lfTone, int lfAmp,
            int hfFreq, boolean hfTone, int hfAmp) {
        long value = 0;
        value |= (long) (lfFreq & 0x1ff);
        value |= (long) (lfTone ? 1 : 0) << 9;
        value |= (long) (clamp(lfAmp, 0, 0x3ff) & 0x3ff) << 10;
        value |= (long) (hfFreq & 0x1ff) << 20;
        value |= (long) (hfTone ? 1 : 0) << 29;
        value |= (long) (clamp(hfAmp, 0, 0x3ff) & 0x3ff) << 30;
        byte[] out = new byte[5];
    ...
    private static void runHapticTest(String mode) {
        ...
        if ("hd".equals(mode)) {
            byte[] left = buildBleVibrationData(0x0e1, false, 320, 0x1e1, false, 460);
            byte[] right = buildBleVibrationData(0x0e1, false, 320, 0x1e1, false, 460);
            updateHdStream(concat(left, right), "self-test");
            sleep(450);
            stopHdStream("self-test-complete");
        ...
        if ("preset".equals(mode)) {
            byte[][] commands = new byte[][] {
                    hexBytes("01 91 01 01 00 04 00 00 00 00 00 00"),
                    hexBytes("0a 91 01 02 00 04 00 00 01 00 00 00"),
                    hexBytes("0a 91 01 02 00 04 00 00 02 00 00 00"),
                    hexBytes("0a 91 01 02 00 04 00 00 03 00 00 00"),
                    hexBytes("0a 91 01 02 00 04 00 00 04 00 00 00"),
                    hexBytes("0a 91 01 02 00 04 00 00 05 00 00 00"),
                    hexBytes("0a 91 01 02 00 04 00 00 00 00 00 00")
            };
    ...
    @SuppressWarnings("deprecation")
    private static void writeCharacteristicNoResponseQuiet(BluetoothGatt gatt, BluetoothGattCharacteristic ch,
            byte[] data, String label) {
        ch.setWriteType(BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE);
        if (Build.VERSION.SDK_INT >= 33) {
            int result = gatt.writeCharacteristic(ch, data, BluetoothGattCharacteristic.WRITE_TYPE_NO_RESPONSE);
            if (result != 0) {
                log(label + " new api failed n=" + data.length + " result=" + result);
            }
        } else {
            ch.setValue(data);
            boolean result = gatt.writeCharacteristic(ch);
            if (!result) {
                log(label + " old api failed n=" + data.length);
            }
        }
    }

    private static final class HdRumbleLoop implements Runnable {
        public void run() {
            long nextLogMs = 0;
            while (running) {
                try {
                    ...
                    byte[] right;
                    synchronized (hdLock) {
                        active = hdStreamActive && nowMs <= hdStreamUntilMs;
                        left = hdLeftVibration;
                        right = hdRightVibration;
                        if (hdStreamActive && !active) {
                            hdStreamActive = false;
                        }
                    }
                    if (active) {
                        sendBleWriteQuiet(CHAR_WRITE_CC48, buildPro2HdPacket(nextHdPacketId(), left, right),
                                "HD stream write");
                        if (nowMs >= nextLogMs) {
                            nextLogMs = nowMs + 500;
                            log("HD stream tick left=" + hex(left, 5) + " right=" + hex(right, 5) +
                                    " untilMs=" + hdStreamUntilMs);
                        }
                        sleep(20);
                    } else {
                        sleep(20);
    ...
    private static void handleInput(byte[] value) {
        ...
        int buttons = value.length >= 8 ? le32(value, 4) : 0;
        int b5 = mapFd2RightButtons(buttons);
        int b6 = mapFd2LeftButtons(buttons);
        int b7 = mapFd2SystemButtons(buttons);
        int b8 = 0;
        int rawB2 = b5;
        int rawB3 = b6;
        int rawB4 = b7;

        int lx = value.length >= 13 ? unpack12X(value, 10) : 2048;
        int ly = value.length >= 13 ? unpack12Y(value, 10) : 2048;
        int rx = value.length >= 16 ? unpack12X(value, 13) : 2048;
        int ry = value.length >= 16 ? unpack12Y(value, 13) : 2048;
        ...
        if (lastInput == null || nowMs - lastSummaryMs >= 250) {
            ...
            log("input n=" + value.length + " count=" + notifyCount +
                    " fd2Buttons=0x" + String.format(Locale.US, "%08x", buttons) +
    ...
    private static void handleLegacyInput(byte[] value) {
        legacyNotifyCount++;
        ...
        if (!calibrated && b5 == 0 && b6 == 0 && b7 == 0) {
            ...
            if (calibrationCount >= 20) {
                ...
                centerRy = (int) (sumRy / calibrationCount);
                calibrated = true;
                log("auto center legacy lx=" + centerLx + " ly=" + centerLy +
                        " rx=" + centerRx + " ry=" + centerRy);
            }
        }
        ...
        long nowMs = System.currentTimeMillis();
        if (lastLegacyInput == null || nowMs - lastLegacySummaryMs >= 250) {
            String delta = lastLegacyInput == null ? "first" : diff(lastLegacyInput, value);
            log("legacy input n=" + value.length + " count=" + legacyNotifyCount +
                    " rawB=" + hexByte(rawB2) + "," + hexByte(rawB3) + "," + hexByte(rawB4) +
                    " mappedB=" + hexByte(b5) + "," + hexByte(b6) + "," + hexByte(b7) +
                    " raw=" + lx + "," + ly + "," + rx + "," + ry +
                    " out=" + outLx + "," + outLy + "," + outRx + "," + outRy +
                    " delta=" + delta);
            lastLegacySummaryMs = nowMs;
            lastLegacyInput = copy(value);
        }
    }

    private static int mapFd2RightButtons(int buttons) {
        int out = 0;
        out |= mapBit(buttons, 0x00000004, 0x01); // B
        out |= mapBit(buttons, 0x00000008, 0x02); // A
        out |= mapBit(buttons, 0x00000001, 0x04); // Y
        out |= mapBit(buttons, 0x00000002, 0x08); // X
        out |= mapBit(buttons, 0x00000040, 0x10); // R
        out |= mapBit(buttons, 0x00000080, 0x20); // ZR
        out |= mapBit(buttons, 0x00000200, 0x40); // Plus
        out |= mapBit(buttons, 0x00000400, 0x80); // Right stick
        return out;
    }

    private static int mapFd2LeftButtons(int buttons) {
        int out = 0;
        out |= mapBit(buttons, 0x00010000, 0x01); // Down
        out |= mapBit(buttons, 0x00040000, 0x02); // Right
        out |= mapBit(buttons, 0x00080000, 0x04); // Left
        out |= mapBit(buttons, 0x00020000, 0x08); // Up
        out |= mapBit(buttons, 0x00400000, 0x10); // L
        out |= mapBit(buttons, 0x00800000, 0x20); // ZL
        out |= mapBit(buttons, 0x00000100, 0x40); // Minus
        out |= mapBit(buttons, 0x00000800, 0x80); // Left stick
        return out;
    }

    private static int mapFd2SystemButtons(int buttons) {
        int out = 0;
        out |= mapBit(buttons, 0x00001000, 0x01); // Home
        out |= mapBit(buttons, 0x00002000, 0x02); // Capture
        out |= mapBit(buttons, 0x01000000, 0x04); // GR
        out |= mapBit(buttons, 0x02000000, 0x08); // GL
        out |= mapBit(buttons, 0x00004000, 0x10); // C
        return out;
    }

    private static int mapBit(int source, int sourceMask, int targetMask) {
        return (source & sourceMask) != 0 ? targetMask : 0;
    }
    ...
    private static int le32(byte[] value, int off) {
        return u(value[off]) | (u(value[off + 1]) << 8) |
                (u(value[off + 2]) << 16) | (u(value[off + 3]) << 24);
    }
