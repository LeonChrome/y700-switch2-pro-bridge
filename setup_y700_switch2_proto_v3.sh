#!/system/bin/sh
#
# v3 Nintendo Switch 2 Pro-compatible USB gadget setup.
# This is separate from setup_y700_switch2_proto.sh and starts
# Switch2FfsResponderV3 with the v3 HD rumble bridge queue.
...
RESPONDER_JAR="${RESPONDER_JAR:-/data/local/tmp/switch2_ffs_responder_v3.jar}"
READY="${READY:-/data/local/tmp/switch2_ffs_v3_ready}"
LOG="${LOG:-/data/local/tmp/switch2_ffs_responder_v3.log}"
...
write_hid64_desc() {
    # Vendor/raw HID with report IDs:
    #   input  report 0x09 + 63 payload bytes
    #   output report 0x02 + 63 payload bytes
...
start_responder() {
    echo "Starting responder v3..."
    [ -f "$RESPONDER_JAR" ] || die "Missing responder jar: $RESPONDER_JAR"
    rm -f "$READY" "$LOG" /data/local/tmp/switch2_ffs_ready
    setsid sh -c "CLASSPATH=$RESPONDER_JAR app_process64 /system/bin Switch2FfsResponderV3 $FFS_DIR /dev/hidg0 >>$LOG 2>&1" >/dev/null 2>&1 &
    ...
    die "Responder v3 did not become ready"
}
