...
$ReleaseRoot = Join-Path $RepoRoot "release\v6.2.18"
...
$SingleExeName = [System.Text.Encoding]::UTF8.GetString([byte[]](
    ...
    0x2E,0x32,0x2E,0x31,0x38,0x2E,0x65,0x78,0x65))
$SingleExe = Join-Path $ReleaseRoot $SingleExeName
$HashFile = Join-Path $ReleaseRoot "SHA256SUMS-v6.2.18.txt"
$DotnetRoot = Join-Path $RepoRoot "work\dotnet"

function Write-Step([string]$Name, [string]$Value) {
    Write-Output "[V6_2_18_PACKAGE] $Name=$Value"
}
...
if (!$DryRun) {
    ...
    Get-ChildItem -LiteralPath $ReleaseRoot -Filter "*v6.2.18*.exe" -ErrorAction SilentlyContinue |
        Remove-Item -Force
    Get-ChildItem -LiteralPath $ReleaseRoot -Filter "SHA256SUMS-v6.2.18*.txt" -ErrorAction SilentlyContinue |
        Remove-Item -Force
