...
$ReleaseRoot = Join-Path $RepoRoot "release\v6.2.20"
...
$SingleExeName = [System.Text.Encoding]::UTF8.GetString([byte[]](
    ...
    0x36,0x2E,0x32,0x2E,0x32,0x30,0x2E,0x65,0x78,0x65))
$SingleExe = Join-Path $ReleaseRoot $SingleExeName
$HashFile = Join-Path $ReleaseRoot "SHA256SUMS-v6.2.20.txt"
$DotnetRoot = Join-Path $RepoRoot "work\dotnet"

function Write-Step([string]$Name, [string]$Value) {
    Write-Output "[V6_2_20_PACKAGE] $Name=$Value"
}
...
if (!$DryRun) {
    ...
    Get-ChildItem -LiteralPath $ReleaseRoot -Filter "*v6.2.20*.exe" -ErrorAction SilentlyContinue |
        Remove-Item -Force
    Get-ChildItem -LiteralPath $ReleaseRoot -Filter "SHA256SUMS-v6.2.20*.txt" -ErrorAction SilentlyContinue |
        Remove-Item -Force
