...
Write-Host "PENDING_HARDWARE_TEST: CH343P detection must be verified when the board arrives."
...
$ports | ForEach-Object {
    $hint = if ($_.Name -match 'CH343|CH340|USB.*Serial|USB-SERIAL|wch|QinHeng') { '  <-- possible CH343/USB serial' } else { '' }
